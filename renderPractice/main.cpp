...
void vector_interp(vector_t *z, const vector_t *x1, const vector_t *x2, float t) {
	z->x = x1->x + ( x2->x - x1->x ) * t;
	z->y = x1->y + ( x2->y - x1->y ) * t;
	z->z = x1->z + ( x2->z - x1->z ) * t;
}
...
void matrix_mul(matrix_t *c, const matrix_t *a, const matrix_t *b) {
	for( int i = 0; i < 4; ++i ){
		for( int j = 0; j < 4; ++j ){
			c->m[i][j] = a->m[i][0] * b->m[0][j] + a->m[i][1] * b->m[j][1] +
...
void matrix_apply(vector_t *y, const vector_t *x, const matrix_t *m) {
	...
	y->x = X * m->m[0][0] + Y * m->m[1][0] + Y * m->m[2][0] + W * m->m[3][0];
	y->y = X * m->m[0][1] + Y * m->m[1][1] + Y * m->m[2][1] + W * m->m[3][1];
	y->z = X * m->m[0][2] + Y * m->m[1][2] + Y * m->m[2][2] + W * m->m[3][2];
	y->w = X * m->m[0][3] + Y * m->m[1][3] + Y * m->m[2][3] + W * m->m[3][3];
}
...
void matrix_set_rotate(matrix_t *m, float x, float y, float z, float theta) {
	...
	m->m[0][0] = x * x *( 1 - cosa ) + cosa;
	m->m[1][0] = x * y * ( 1 - cosa ) - z * sina;
	m->m[2][0] = x * z * ( 1 - cosa ) + y * sina;

	m->m[0][1] = x * y * ( 1 - cosa ) + z * sina;
	m->m[1][1] = y * y * ( 1 - cosa ) + cosa;
	m->m[2][1] = y * z * ( 1 - cosa ) - x * sina;

	m->m[0][2] = x * z * ( 1 - cosa ) - y * sina;
	m->m[1][2] = y * z * ( 1 - cosa ) + x * sina;
	m->m[2][2] = z * z * ( 1 - cosa ) + cosa;

	m->m[0][3] = m->m[1][3] = m->m[2][3] = m->m[3][0]
	=m->m[3][1] = m->m[3][2] = 0.0f;
	m->m[3][3] = 1.0f;
}
...
void matrix_set_lookat(matrix_t *m, const vector_t *eye, const vector_t *at, const vector_t *up) {
	...
	vector_crossproduct( &yaxis, eye, &zaxis );
...
int main(void)
{
	...
	TCHAR *title = _T("CX3d (software render tutorial) - ")
